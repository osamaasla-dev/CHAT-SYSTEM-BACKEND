...
export class DeleteContactService {
  private readonly logger = new Logger(DeleteContactService.name);
  ...
  async execute(ownerId: string, contactId: string): Promise<void> {
    if (ownerId === contactId) {
      this.logger.warn('Invalid contact delete: ownerId equals contactId');
    ...
    await this.rateLimitService.enforceRateLimit({
      ...
      limit,
      windowSeconds,
      logContext: `contacts:delete ownerId=${ownerId} contactId=${contactId}`,
    });

    this.logger.log(
      `Attempting to delete contact: ownerId=${ownerId}, contactId=${contactId} started`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Failed to delete contact: ownerId=${ownerId}, contactId=${contactId}`,
        error,
      );
    ...
    this.logger.log(
      `Contact deleted successfully: ownerId=${ownerId}, contactId=${contactId}`,
    );
  }
}
